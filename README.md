# Random Password Generator

**Автор:** Плумаки Анна Янис

**GitHub:** https://github.com/plumakianna-prog/password-generator

## Описание программы

Random Password Generator — это графическое приложение (GUI) на Python с использованием библиотеки tkinter.  
Программа позволяет генерировать случайные пароли с настраиваемыми параметрами:

- длина пароля (от 4 до 64 символов)
- использование цифр
- использование букв (A-Z, a-z)
- использование спецсимволов (!@#$%^&*)

Все сгенерированные пароли сохраняются в историю (файл `password_history.json`) с указанием длины и даты создания.

## Требования для запуска

- Python 3.7 или выше
- Встроенные библиотеки: `tkinter`, `random`, `string`, `json`, `datetime` — дополнительной установки не требуют

## Как запустить

```bash
git clone https://github.com/plumakianna-prog/password-generator.git
cd password-generator
python password_generator.py
```
## Как пользоваться

Выберите длину пароля с помощью ползунка.
Отметьте нужные типы символов (цифры, буквы, спецсимволы).
Нажмите кнопку «Сгенерировать пароль».
Пароль появится в текстовом поле.
Все сгенерированные пароли отобразятся в таблице «История паролей».
При необходимости можно очистить историю.

## Примеры работы

### Пример 1: простой пароль только из цифр

| Параметр | Значение |
|----------|----------|
| Длина | 6 |
| Цифры | ✅ |
| Буквы | ❌ |
| Спецсимволы | ❌ |

**Результат:** `482739`

---

### Пример 2: сложный пароль

| Параметр | Значение |
|----------|----------|
| Длина | 16 |
| Цифры | ✅ |
| Буквы | ✅ |
| Спецсимволы | ✅ |

**Результат:** `aB3!xL9@qR8#mN2$`

---

### Пример 3: ошибка при пустом наборе символов

Если не выбрать ни один тип символов и нажать «Сгенерировать пароль»,  
программа покажет сообщение об ошибке: `"Ошибка: выберите хотя бы один тип символов!`

## Управление историей

### Очистка истории
Нажмите кнопку «Очистить историю» в нижней части окна.
Подтвердите действие в диалоговом окне.
История будет полностью удалена из интерфейса и файла password_history.json.
